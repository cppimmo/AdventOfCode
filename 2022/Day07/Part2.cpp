...
constexpr int kTOTAL_SPACE = 70'000'000;
constexpr int kEMPTY_SPACE_NEEDED = 30'000'000;
...
int ProcessShellSession(std::ifstream &input)
{
	...
	const int kCurrUnusedSpace = kTOTAL_SPACE - dirSizes["/"];
	const int kSizeToDelete = kEMPTY_SPACE_NEEDED - kCurrUnusedSpace;

	std::vector<int> sortedSizes;

	for (const auto &[key, value] : dirSizes)
		sortedSizes.push_back(value);

	std::sort(sortedSizes.begin(), sortedSizes.end(), std::less<int>());

	for (const auto &elem : sortedSizes)
		std::cout << elem << '\n';

	auto findIter = std::find_if(sortedSizes.begin(), sortedSizes.end(),
								 [kSizeToDelete](const auto &elem) -> bool
								 {
									 return (elem >= kSizeToDelete);
								 });
